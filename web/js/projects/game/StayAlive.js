...
$$StayAlive = new function() {
    ...
    this.init = function() {
        ...
        if (canvas && canvas.getContext) {
            ...
            bestScoreInfo.setText($$.cookie("StayAliveBestScore") || 0);
            loadSound();
    ...
    function resize() {
        ...
        if (isMobile) {
            currentVelocity = startVelocity = 1.5 * startVelocity;
        }
        game.css({
            "margin": "0",
    ...
    function gameOver() {
        if (life) {
            ...
            bestScoreInfo.setText($$.cookie("StayAliveBestScore"));
            if (parseInt($$.cookie("StayAliveBestScore") || 0) < score) $$.cookie("StayAliveBestScore", score);
            sound("ready");
        }
    }
    ...
    function playGame() {
        ...
        for (var i = 0; i < mines.length; i++) {
            if (!((mines[i].position.x < -10 || mines[i].position.y > canvasHeight + 10) && mines.splice(i, 1))) {
                mines[i].position.x -= mines[i].veriocity * currentVelocity;
                mines[i].position.y += mines[i].veriocity * currentVelocity;
        ...
        for (i = 0; i < snow.length; i++) {
            snow[i].position.x += (snow[i].xVeocity - currentVelocity);
            snow[i].position.y += (snow[i].yVeocity + currentVelocity);
        ...
        shield.position.x -= currentVelocity;
        shield.position.y += currentVelocity;
        if (shield.position.x < -10 || shield.position.y > canvasHeight + 10) {
            shield.life = false;
        } else if (shield.life) {
            ctx.fillStyle = "rgba(123, 11, 175, .1)";
            for (i = 0; i < shield.size; i += 0.5) {
                ctx.beginPath();
        ...
        if (life) {
            ...
            //check collision with bonus BGC
            ...
            //check player is over side canvas
            ...
            for (i = 0; i < tail.length; i++) {
                tail[i].position.x -= currentVelocity;
                tail[i].position.y += currentVelocity;
